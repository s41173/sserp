...
class Color extends MX_Controller
{
    ...
    {
        if(!$search){ $result = $this->Color_model->get_last($this->modul['limit'])->result(); }
    ...
    {
       $val = $this->Color_model->get_primary()->row();
       $lng = array('primary' => 0);
       $this->Color_model->update($val->id,$lng);

       $lng = array('primary' => 1);
       $this->Color_model->update($uid,$lng);
    }
    ...
    {
      $val = $this->Color_model->get_by_id($uid)->row();
    ...
    {        
        $category = $this->Color_model->get_by_id($uid)->row();
//
...
//        $this->load->view('category_update', $data);
    ...
    function valid($code)
    {
        if ($this->Color_model->valid('name',$code) == FALSE)
        {
	...
	if ($this->Color_model->validating('name',$code,$id) == FALSE)
        {
	...
	// Form validation
	    ...
	    $this->Color_model->update($this->session->userdata('langid'), $category);
