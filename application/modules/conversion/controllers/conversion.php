...
class Conversion extends MX_Controller
{
    function __construct()
    {
        ...
        $this->load->model('Conversion_model', '', TRUE);
    ...
    {
        if(!$search){ $result = $this->Conversion_model->get_last($this->modul['limit'])->result(); }
        
        if ($result){
	foreach($result as $res)
	{   
	   $output[] = array ($res->id, $res->num, $res->alpha,
    ...
    function get_last()
    {
        ...
        $data['main_view'] = 'conversion_view';
	...
	// ---------------------------------------- //
        ...
        $this->table->set_heading('#', 'Num', 'Alpha', 'Action');
    ...
    {
       $val = $this->Conversion_model->get_primary()->row();
       $lng = array('primary' => 0);
       $this->Conversion_model->update($val->id,$lng);

       $lng = array('primary' => 1);
       $this->Conversion_model->update($uid,$lng);
    }
    ...
    function delete_all()
    {
      ...
      if($cek)
      {
        ...
        for ($i=0; $i<$jumlah; $i++)
        {
           ...
           {
              $this->Conversion_model->delete($cek[$i]); 
           }
           else { $x=$x+1; }
    ...
    {
      $val = $this->Conversion_model->get_by_id($uid)->row();
    ...
    function delete($uid,$type='soft')
    {
        ...
        if ($type == 'soft'){
           $this->Conversion_model->delete($uid);
	...
	// Form validation
        ...
        $this->form_validation->set_rules('tcode', 'Code', 'required|callback_valid');
        ...
        if ($this->form_validation->run($this) == TRUE)
        {
            ...
            $this->Conversion_model->add($category);
    ...
    {        
        $category = $this->Conversion_model->get_by_id($uid)->row();
//
	$this->session->set_userdata('langid', $category->id);
//        $this->load->view('category_update', $data);
        
        echo $uid.'|'.$category->num.'|'.$category->alpha;
    }

    function valid($code)
    {
        if ($this->Conversion_model->valid('code',$code) == FALSE)
        {
	...
	if ($this->Conversion_model->validating('alpha',$code,$id) == FALSE)
        {
            $this->form_validation->set_message('validation', 'This conversion is already registered!');
            return FALSE;
        }
        else{ return TRUE; }
    }
	...
	// Form validation
        $this->form_validation->set_rules('talpha', 'Alpha', 'required|callback_validation');

        if ($this->form_validation->run($this) == TRUE)
        {
            $category = array('alpha' => strtoupper($this->input->post('talpha')));

	    $this->Conversion_model->update($this->session->userdata('langid'), $category);
