...
class Contract extends MX_Controller
{
    ...
    private $properti, $modul, $title, $api, $customer, $vendor;
    ...
    public function getdatatable($search=null,$date='null',$cust='null',$type='null')
    {
	 ...
	 {
           if ($res->status == 0){ $status = 'EX'; }else{ $status = 'AC'; }
	   $output[] = array ($res->id, tglin($res->dates), $res->contractno, $this->customer->get_name($res->cust_id), tglin($res->starts).' <br> '.tglin($res->ends), $res->approved, $status);
	 } 
	...
	// ---------------------------------------- //
        ...
        $this->table->set_heading('#','No', 'Date', 'Doc-No', 'Customer', 'Period', 'Status', 'Action');
...
//        $data['default']['api'] = $product->unique_id;
	...
	if ($this->model->validating('model',$val,$id) == FALSE)
     ...
     private function update_tank_api($mid=0){
        ...
        $tank = $this->model->get_by_id($mid)->row();
        $param = '{ "namespace":"'.clientcode.'", "kode":"'.$tank->sku.'", "hRef":"'.floatval($tank->measuring/100).'", "teraTemp":"'.$tank->temperature.'", "koefMuaiRuang":"'.$tank->coeff.'", "massaJenisCairan":"'.$tank->density.'" }';
        $response = $this->api->request(api.'/'.$tank->unique_id, $param, 'code','PUT');
        
        $result = (array) json_decode($response[0], true);
        if ($response[1] == 202){return TRUE;}else{ return FALSE; }
    }
        ...
        elseif ($param == 2)
        {
            ...
            if ($this->form_validation->run($this) == TRUE)
            {
                ...
                $product = array('dimension' => $this->input->post('tdimension'),
                                 ...
                                 'extra_percentage' => $this->input->post('textrapercent'),
